    ...
    <div className={`flex min-h-screen ${bgMain}`}>
      ...
      <div className="flex-1 lg:ml-[20vw]">
        ...
        <div className="p-6">
          ...
          {/* --- NEW: AESTHETIC PROGRESS SECTION --- */}
          {isLoading && (
            <div className={`border rounded-2xl overflow-hidden shadow-lg mb-6 ${cardBg}`}>
              ...
              <div className="p-6">
                 <div className="flex justify-between text-sm mb-2">
                    <span className={textSecondary}>Processed Records</span>
                    <span className="font-bold">{results.length}</span>
                 </div>
                 {/* Indeterminate Progress Bar (since total is unknown until end) */}
                 <div className="w-full h-2 bg-gray-200 rounded-full overflow-hidden relative">
                    <div className="absolute top-0 left-0 h-full bg-indigo-500 w-1/3 animate-[shimmer_1.5s_infinite_linear] rounded-full" 
                         style={{backgroundImage: 'linear-gradient(to right, transparent, rgba(255,255,255,0.5), transparent)'}}>
                    </div>
                    <div className="h-full bg-indigo-600 transition-all duration-300 rounded-full" style={{ width: `${Math.min((results.length * 5), 100)}%` }}></div>
                 </div>
              </div>
              ...
              <div 
                ref={logContainerRef}
                className={`h-72 overflow-y-auto font-mono text-xs border-t ${Dark ? "bg-gray-900 border-gray-700" : "bg-slate-900 border-gray-200"}`}
              >
                 {log.length === 0 && (
