...
async def validate_file(file: UploadFile = File(...)):
    ...
    async def file_processor_stream():
        ...
        try:
            ...
            print("📄 Uploaded file suffixes:", suffixes)
            ...
            # ======================
                ...
                yield f"data: {json.dumps({'type': 'log', 'content': '🖼️ Running OCR (OpenAI Vision)...'})}\n\n"

                provider_list = run_ocr(temp_filename)

                if not isinstance(provider_list, list):
                    yield f"data: {json.dumps({'type': 'log', 'content': '❌ OCR returned invalid data format'})}\n\n"
                    provider_list = []

                elif provider_list and isinstance(provider_list[0], dict) and provider_list[0].get("error"):
                    error_msg = provider_list[0].get("error", "Unknown OCR error")
                    yield f"data: {json.dumps({'type': 'log', 'content': f'❌ OCR Error: {error_msg}'})}\n\n"
                    provider_list = []
            ...
            if total_records == 0:
                yield f"data: {json.dumps({'type': 'log', 'content': '⚠️ No providers extracted. Please check document quality.'})}\n\n"
            ...
            async def worker(provider_info, index):
                try:
                    provider_name = (
                        ...
                        or provider_info.get("fullName")
                        or "OCR Extracted Provider"
                    )
